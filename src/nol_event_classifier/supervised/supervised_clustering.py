...
MODELS = [
    ...
    "setfit_paraphrase-multilingual-mpnet-base-v2_1ep-16bs-1it",
...
class SupervisedClustering:
    ...
    def export_results(self, results, model_name):
        ...
        for r in results:
            rows.append({
                ...
                "status": "confident" if r["best_score"] >= self.threshold else "uncertain",
        ...
        log(f"  -> {csv_path}")
        log(f"     {n_confident} Confidents / {n_uncertain} Uncertains (threshold={self.threshold})")

        return df

    def display_clusters_graph(self, df, embeddings, model_name, show=False, min_label_count=2, max_text_labels=60):
    ...
    def flatten_events(self, event_list_raw):
        flattened_events = []
